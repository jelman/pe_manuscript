...
title: 'Practice Effects on Cognitive Tests and AFQT Tests: 
        Unadjusted, raw test score scale'
author: "Jeremy Elman"
date: '`r Sys.Date()`'
output:
...
```{r, include=F}
# Load data
allDat = read.csv("~/netshare/K/Projects/PracticeEffects/data/V1V2_CogData_Unadj_Z.csv")
...
1. The scores are unadjusted raw scores. 
2. Practice effects are calculated from scores on the raw score scale.
3. Practice effects for cognitive domains are created by averaging the practice effects from all tests within a domain.
...
calcPracticeEffect = function(dat, varName, idxS1T2, idxS2T2,idxS1T1ret){
  ...
  varV1 = paste0(varV1,"_z")
  varV2 = paste0(varV2,"_z")
...
calcCogDomains = function(testdat){
  testdat[['VisSpat']] = mean(abs(testdat[c("MR1COR","HFTOTCOR")]))
  testdat[['AbsReason']] = abs(testdat['MTXRAW'])
  testdat[['STWKMem']] = mean(abs(testdat[c("DSFRAW","DSBRAW","LNTOT","SSPFRAW",
                                              "SSPBRAW","RSATOT")]))
  testdat[['EpsMem']] = mean(abs(testdat[c("CVATOT","CVSDFR","CVLDFR","LMITOT",
                                               "LMDTOT","VRITOT","VRDTOT")]))
  testdat[['VerbFlu']] = mean(abs(testdat[c("LFCOR","CFCOR")]))
  testdat[['ProcSpeed']] = mean(abs(testdat[c("STRWRAW","STRCRAW","TRL2TLOG","TRL3TLOG")]))
  testdat[['ExecTrailsSwitch']] = abs(testdat['TRL4TLOG'])
  testdat[['ExecCategorySwitch']] = abs(testdat['CSSACC'])
  testdat
}
```
...
write.csv(results, '~/netshare/K/Projects/PracticeEffects/results/PracEffectValues_Unadj_Z.csv')
```
